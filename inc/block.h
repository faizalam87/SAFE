...
class BLOCK {
  public:
    ...
    uint64_t address, //line address after removing last 6 bits
    ...
    // replacement state
    uint32_t lru;
    // in struct BLOCK (memory_class.h)
    bool was_prefetch_fill ;   // set on fill if the incoming packet is a prefetch
    bool demand_used;   // set true on first demand hit to this line
	...
	ip = 0;
        ...
        lru = 0;
            // in struct BLOCK 
        was_prefetch_fill = false;   // set on fill if the incoming packet is a prefetch
        demand_used       = false;   // set true on first demand hit to this line
    };
};
