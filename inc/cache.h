...
struct BTC_Entry {
    uint64_t tag;        // The object ID or upper bits of the pointer
    uint64_t base_addr;  // The start of the allocated object
    uint64_t bound_addr; // The end of the allocated object
    bool valid;          // Is this entry active?
    uint32_t lru;        // LRU replacement counter
};

class BoundTableCache {
private:
    uint32_t num_sets;
    uint32_t num_ways;
    BTC_Entry** entries;

    // Simple hash function to find the set
    uint32_t get_set(uint64_t tag) {
        return tag % num_sets; 
    }

    // Standard LRU update logic
    void update_lru(uint32_t set, uint32_t accessed_way) {
        uint32_t old_lru = entries[set][accessed_way].lru;
        for (uint32_t way = 0; way < num_ways; ++way) {
            if (entries[set][way].valid && entries[set][way].lru < old_lru) {
                entries[set][way].lru++;
            }
        }
        entries[set][accessed_way].lru = 0; // 0 is Most Recently Used
    }

public:
    // Statistics for your paper
    uint64_t hits;
    uint64_t misses;

    BoundTableCache(uint32_t sets, uint32_t ways) : num_sets(sets), num_ways(ways) {
        hits = 0;
        misses = 0;
        entries = new BTC_Entry*[num_sets];
        for (uint32_t i = 0; i < num_sets; i++) {
            entries[i] = new BTC_Entry[num_ways];
            for (uint32_t j = 0; j < num_ways; j++) {
                entries[i][j].valid = false;
                entries[i][j].lru = j; // Initialize LRU
            }
        }
    }

    ~BoundTableCache() {
        for (uint32_t i = 0; i < num_sets; i++) delete[] entries[i];
        delete[] entries;
    }

    // Retrieve: Returns true on HIT, false on MISS. Populates base/bound on HIT.
    bool lookup(uint64_t tag, uint64_t &base, uint64_t &bound) {
        uint32_t set = get_set(tag);
        
        for (uint32_t way = 0; way < num_ways; ++way) {
            if (entries[set][way].valid && entries[set][way].tag == tag) {
                hits++;
                //std::cout<< "BTC hit" << hits << std::endl;
                base = entries[set][way].base_addr;
                bound = entries[set][way].bound_addr;
                update_lru(set, way);
                return true; // HIT
            }
        }
        misses++;
        //std::cout<< "BTC miss" << misses << std::endl;
        return false; // MISS
    }

    // Insert: Puts a new Base/Bound into the cache, evicting LRU if full
    void insert(uint64_t tag, uint64_t base, uint64_t bound) {
        uint32_t set = get_set(tag);
        uint32_t victim_way = 0;
        
        // Find an invalid way, or fallback to the LRU way (lru == num_ways - 1)
        for (uint32_t way = 0; way < num_ways; ++way) {
            if (!entries[set][way].valid) {
                victim_way = way;
                break;
            }
            if (entries[set][way].lru == num_ways - 1) {
                victim_way = way;
            }
        }

        // Install new entry
        entries[set][victim_way].valid = true;
        entries[set][victim_way].tag = tag;
        entries[set][victim_way].base_addr = base;
        entries[set][victim_way].bound_addr = bound;
...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t  prefetch_request=0;
    uint64_t  prefetch_hit_outside_bound=0;
    uint64_t  prefetch_hit_inside_bound=0;
    uint64_t  prefetch_miss_outside_bound=0;
    uint64_t  prefetch_miss_inside_bound=0;
    // queues
    ...
    uint64_t pollution_misses; // # demand misses attributed to prefetch-caused evictions
    uint64_t evictions_by_pf;   // # useful lines evicted by a prefetch fill
    uint64_t pf_killed_by_heap_guard;
    ...
    // one small ghost list per set
   std::deque<GhostEntry>* pf_ghost = nullptr;

   // (optional) small helpers, defined in cache.cc
   void ghost_insert(uint32_t set, uint64_t tag);
   bool ghost_hit_and_consume(uint32_t set, uint64_t tag);
   void ghost_age_out(uint32_t set, uint64_t now);
...
//	mshr_full = 0;
    ...
    pollution_misses=0; // # demand misses attributed to prefetch-caused evictions
    evictions_by_pf=0;   // # useful lines evicted by a prefetch fill
    pf_killed_by_heap_guard=0;
    pf_ghost = new std::deque<GhostEntry>[NUM_SET];
