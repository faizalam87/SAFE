...
extern void print_live_heap(std::ostream& os);
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
	...
	//}
			cout << cache->NAME << " PREFETCHES SAME FILL-ORIGIN LEVEL: " << cache->pf_same_fill_level << " DIFFERENT FILL-ORIGIN LEVEL: " << cache->pf_lower_fill_level << endl;
                 ...
                 //ADDED     
                    cout << cache->NAME  << " PREFETCH_POLLUTION_MISSES: " << cache->pollution_misses      << "  PF_EVICTIONS_OF_USEFUL_LINES: " << cache->evictions_by_pf  << "  NET_PREFETCH_BENEFIT: "   << (cache->pf_useful > cache->pollution_misses ? (cache->pf_useful - cache->pollution_misses) : 0)   << " PF_KILLED_BY_HEAP_GUARD: " << cache->pf_killed_by_heap_guard  << endl;
                  
           // ADDED FOR BTC STATS
                    if (cache->btc != nullptr) {
                        cout << cache->NAME << " BTC_HITS: " << cache->btc->hits << "  BTC_MISSES: " << cache->btc->misses << endl;
                    }
...
//	}
    ...
    if (NUM_CPUS > 1) {
	   ...
	   // print_live_heap(std::cout); //Added live heap dumps
	    //@Vishal: print stats
	    cout<<endl;
