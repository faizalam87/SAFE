...
int asid_index=0;
uint64_t obj_small_than_block=0;
uint64_t pf_in_other_object=0;
uint64_t pf_in_same_object=0;
uint64_t pf_killed=0;
uint64_t L1_pf_access=0;
uint64_t base_not_in_heap=0;

int reg_instruction_pointer = REG_INSTRUCTION_POINTER, reg_flags = REG_FLAGS, reg_stack_pointer = REG_STACK_POINTER;

std::map<uint64_t, uint64_t> live_heap; // start -> inclusive_end

// true if a is in some live heap object; returns that object's start/end
bool heap_lookup(uint64_t a, uint64_t& start, uint64_t& end) {
    auto it = live_heap.upper_bound(a); // first start > a
    if (it == live_heap.begin()) {
        //std::cout << "[HEAP-LKUP] a=0x" << std::hex << a
        //          << " -> MISS (no range with start<=a)\n" << std::dec;
        return false;                    // empty map or a before first range
    }
    ...
    return hit;                          // inside that [start,end] ?
}
// true if base_addr and pf_addr are inside the same heap object
// Requires: heap_lookup(base_addr, start, end) that returns true if base_addr is inside [start,end]

bool same_heap_object(uint64_t base_addr, uint64_t pf_addr)
{
    L1_pf_access++;
    ...
    if (!heap_lookup(base_addr, start, end))  // base not a heap obj ⇒ allow
    {

        base_not_in_heap++;
      //  std::cout << "Base not in Heap" << std::endl;
        return true;
    }
    // Address found in the heap and then they are same
    if(start==end)

      {
    ...
    
       {
            ...
            obj_small_than_block++;
        //    std::cout << "[PF-SB] object_smaller_than_block =[0x" << base_addr << ",0x" << start << ",0x" << end << "]" << "\n";

        }


     } 
    //if base_addr start<= base_add <= end
    // Primary, byte-level containment: pf must be within [start,end].
    if (pf_addr >= start && pf_addr <= end)
    {
        pf_in_same_object++;
     //   std::cout << "[PF-SO] pf_in_same_object =[0x" << pf_addr << ",0x" << start << ",0x" << end << "]" << "\n";
        return true;
    }
        

    // Secondary check: pf_addr either isn't a heap addr or belongs to a *different* object.
    //uint64_t s2, e2;
    //bool pf_in_heap = heap_lookup(pf_addr, s2, e2);

           
    //if (pf_in_heap)
    //{
    //   pf_in_other_object++;
    //   std::cout << "[PF-OO] pf_in_other_object[0x" << pf_addr << ",0x" << start << ",0x" << end << "]" << "\n";
    //   return true;
    //}
    
   // std::cout << "[HEAP-GUARD] kill pf obj=[0x"
   //           << std::hex << start << ",0x" << end
   //           << "] base=0x" << base_addr
   //           << " pf=0x" << pf_addr;
   // std::cout << std::dec << "\n"; 
    pf_killed++;
...
void print_live_heap(std::ostream& os)
{
    std::ios old_state(nullptr);
    old_state.copyfmt(os);           // save stream formatting

    os << "base_hex,end_hex,size_bytes\n";
    uint64_t total_bytes = 0;
    for (const auto& [start, end] : live_heap) {
        const uint64_t size = (end >= start) ? (end - start + 1) : 0;
        total_bytes += size;
        os << "0x" << std::hex << start << ",0x" << end
           << std::dec << "," << size << "\n";
    }
    os << "#live_objects=" << live_heap.size()
       << ", total_bytes=" << total_bytes << "\n";
    
    os << "#obj_small_than_block =" << obj_small_than_block
       << ", #L1_pf_access=" << L1_pf_access 
       << ", #pf_in_same_object=" << pf_in_same_object 
       << ", #base_not_in_heap=" << base_not_in_heap  
       << ", #pf_in_other_object=" << pf_in_other_object
       << ", #pf_killed=" << pf_killed << "\n"; 
    os.copyfmt(old_state);           // restore formatting
}
...
//Neelu: Addition end
	...
	else
	  {
		...
		input_instr trace_read_instr;
            ...
            // Keep reading until we get a real instruction (alloc_type == 0)
            for (;;) {
                if (!fread(&trace_read_instr, instr_size, 1, trace_file)) {
                    // EOF: reopen and try again
                    std::cout << "*** Reached end of trace for Core: " << cpu
                            << " Repeating trace: " << trace_string << std::endl;
                    pclose(trace_file);
                    trace_file = popen(gunzip_command, "r");
                    if (trace_file == NULL) {
                ...
                // --- Handle metadata records and continue reading ---
                if (trace_read_instr.alloc_type == 3) {
                    // Live-at-skip snapshot: collect summary if you want
                    // e.g., live_snapshot_count++, live_snapshot_bytes += trace_read_instr.alloc_size;
                    const uint64_t start = trace_read_instr.alloc_start;
                    const uint64_t end   = trace_read_instr.alloc_end;   // inclusive
                    if (end >= start) live_heap[start] = end;            // ignore weird zero/neg
                      continue; // not a real instruction
                }

                if (trace_read_instr.alloc_type == 1) {
                    const uint64_t start = trace_read_instr.alloc_start;
                    const uint64_t end   = trace_read_instr.alloc_end;   // inclusive
                    if (end >= start) live_heap[start] = end;            // ignore weird zero/neg
                        continue; // not a real instruction
                ...
                if (trace_read_instr.alloc_type == 2) {
                    const uint64_t start = trace_read_instr.alloc_start;
                    live_heap.erase(start);
                    
                               // free by original start
                    continue; // not a real instruction
                }

                // If we’re here, alloc_type == 0 → real instruction. Use it below.
                break;
            }
            ...
            if(trace_read_instr.alloc_type == 3)
            {
                std::cout << "Breaking :: " << "Alloc_Address" << std::hex <<trace_read_instr.alloc_start << std::endl;
                continue;
            }
            if (trace_read_instr.alloc_type == 1 ) {
             // New allocation
            uint32_t obj_blk_start = static_cast<uint32_t>(trace_read_instr.alloc_start);
            uint32_t obj_blk_end = static_cast<uint32_t>(trace_read_instr.alloc_end);
            malloc_map[obj_blk_start] = obj_blk_end;
            instruction_count++;
            //std::cout << "Instruction malloc : " << instruction_count << std::endl;
            continue;
            }
            ...
            // Deallocation
            uint32_t obj_blk_start = static_cast<uint32_t>(trace_read_instr.alloc_start);
            if (malloc_map.find(obj_blk_start) != malloc_map.end()) {
                malloc_map.erase(obj_blk_start);
            }
            instruction_count++;
            //std::cout << "Instruction free : " << instruction_count << std::endl;
            continue;
            }
             */
            ...
            //else { // successfully read the trace
            instruction_count++;
            //std::cout << "Instruction normal : " << instruction_count << std::endl;
