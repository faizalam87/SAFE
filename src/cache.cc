...
uint64_t l1D_prefetch=0;
extern bool same_heap_object(uint64_t base_addr, uint64_t pf_addr);
extern void dump_live_heap();            // if you want to call the printer
...
// --- Prefetch-pollution helpers ---------------------------------------------

// Tag = block address / #sets (set = block address % #sets)
static inline uint64_t addr_to_tag(uint64_t block_addr, uint32_t num_sets) {
    return block_addr / num_sets;
}

// Keep ghost list small + optional aging
static constexpr uint32_t GHOST_CAP = 128;
static constexpr uint64_t GHOST_TTL = 50000; // cycles; tune if you want

void CACHE::ghost_insert(uint32_t set, uint64_t tag) {
    if (pf_ghost) {
   // std::cout << "set=" << set << " size=" << pf_ghost[0].size() << "\n";
    }
   // std::cout << "Set:" << set << "  tag: " << tag << std::endl;
    auto &dq = pf_ghost[set];
   // if (dq.size() >= GHOST_CAP) 
    //{
   //     dq.pop_back();         // LRU out
   //     std::cout << "GHOST CAP is small" << std::endl;
   // }
    dq.push_front(GhostEntry{tag, current_core_cycle[cpu], false}); // MRU in
}

bool CACHE::ghost_hit_and_consume(uint32_t set, uint64_t tag) {
    //std::cout << "[LOOKUP] ghost_hit_and_consume set=" << set 
    //          << " lookup_tag=0x" << std::hex << tag << std::dec << "\n";
    auto &dq = pf_ghost[set];
    for (auto it = dq.begin(); it != dq.end(); ++it) {
        //std::cout << "    checking entry tag=0x" << std::hex << it->tag 
        //          << " counted=" << it->counted << std::dec << "\n";
        if (it->tag == tag) {
            // count only once
    ...
    //std::cout << "    no match in ghost set\n";
    return false;
}

void CACHE::ghost_age_out(uint32_t set, uint64_t now) {
    auto &dq = pf_ghost[set];
    while (!dq.empty() && (now - dq.back().born_cycle) > GHOST_TTL) dq.pop_back();
}
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (do_fill){
            ...
            if (cache_type == IS_L1D)
            {
                ...
                //std::cout << std::hex << "v_fill_addr=0x" << v_fill_addr
               // << " v_evicted_addr=0x" << v_evicted_addr
               // << "Full Address=0x" << MSHR.entry[mshr_index].full_addr
               // << std::dec << '\n';

                bool incoming_is_prefetch = (MSHR.entry[mshr_index].type == PREFETCH);
                if (incoming_is_prefetch && block[set][way].valid) {
                    evictions_by_pf++;
                    uint64_t full_address = v_evicted_addr; //evicted virtual address
                    uint64_t block_adress = full_address>>LOG2_BLOCK_SIZE;
                    ghost_insert(0, block_adress);
            ...
            // ADDED: if a prefetch fill is about to evict a *useful* line, remember its tag
           /*
            bool incoming_is_prefetch = (MSHR.entry[mshr_index].type == PREFETCH);
            if ((cache_type == IS_L1D ) && incoming_is_prefetch && block[set][way].valid) {
                if (block[set][way].demand_used) {
                    evictions_by_pf++;
                  
          
                    uint64_t full_address = block[set][way].full_addr;
                    uint64_t block_adress = full_address>>LOG2_BLOCK_SIZE;
                    uint64_t tag_address = block_adress >> static_cast<int>(std::log2(NUM_SET));
                   // std::cout << "INSERT" 
                   // << " tag_address=0x" << std::hex << tag_address
                   // << " addr=0x" << block[set][way].address
                   // << " full_address=0x" << full_address << std::dec << "\n";
                    ghost_insert(0, tag_address);
                }
           ...
           */
            ...
            // ADDED: annotate the freshly-filled line
            //block[set][way].was_prefetch_fill = incoming_is_prefetch;
            //block[set][way].demand_used       = false;   // reset for the new occupant
...
if (writeback_cpu == NUM_CPUS)
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            // ADDED: RFO hit is also a demand use
           // if ((cache_type == IS_L1D || cache_type == IS_L2C || cache_type == IS_LLC) &&
          //      (WQ.entry[index].type == RFO)) {
          //      block[set][way].demand_used = true;
          //  }

           
           /*
           This block is redundant as Read miss conatins both LOAD and RFO. 
            if ((cache_type == IS_L1D ) && WQ.entry[index].type == RFO) {
                      
                        uint64_t miss_tag = WQ.entry[index].full_virtual_address;
                        uint64_t block_address = miss_tag>>LOG2_BLOCK_SIZE;
                        uint64_t tag_address = block_address>> static_cast<int>(std::log2(NUM_SET));
                        //std::cout << "[LOOKUP] " 
                        //        << " tag_address=0x" << std::hex << tag_address  
                        //        << " full virtual address=0x" << miss_tag << std::dec << "\n";
                        if (ghost_hit_and_consume(0, tag_address)) {
                            pollution_misses++;
                        }
                    }
            */
                ...
                if (do_fill) {
                    ...
                    else if (cache_type == IS_L1D)
                    {
                        ...
                        l1d_prefetcher_cache_fill(v_fill_addr, WQ.entry[index].full_addr, set, way, 0, v_evicted_addr, block[set][way].address<<LOG2_BLOCK_SIZE, WQ.entry[index].pf_metadata);
                           ...
                           // This function is never called and tthe RFO updates in done 
                            bool incoming_is_prefetch = (WQ.entry[index].type == PREFETCH);
                            if (incoming_is_prefetch && block[set][way].valid) {
                        // if (block[set][way].demand_used) {
                                evictions_by_pf++;
                            ...
                            //  std::cout << "INSERT" 
                            // << " tag_address=0x" << std::hex << tag_address
                            // << " VA evicted addr=0x" << v_evicted_addr
                            // << " VA fill address =0x" << v_fill_addr << std::dec << "\n";
                                ghost_insert(0, tag_address);
                            //}
                            }   		      
                          */
                    ...
                    bool incoming_is_prefetch = false; // writeback path never a prefetch fill
                    // no ghost_insert here (only prefetch fills can be blamed)

                    fill_cache(set, way, &WQ.entry[index]);
                
                    block[set][way].was_prefetch_fill = false;
                    block[set][way].demand_used       = (WQ.entry[index].type == RFO) ? true : false; // usually false
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                if (RQ.entry[index].type == LOAD) {
                    ...
                    if (cache_type == IS_L1D) {
                        // Full Address here is virtual address
                       ...
                       // std::cout << "[L1D TRAIN HIT] "
                       // << "  VA.full=0x" << std::hex << RQ.entry[index].full_addr
                       // << "  PA.full=0x" << RQ.entry[index].full_physical_address
                       // << "  ip=0x"      << RQ.entry[index].ip
                       // << std::dec << "\n";
                ...
                // ADDED: mark line as used by demand (data or instr) on a hit
               // if ((cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C || cache_type == IS_LLC) &&
              //      (RQ.entry[index].type == LOAD || RQ.entry[index].type == RFO)) {
              //      block[set][way].demand_used = true;
              //  }
                ...
                reads_available_this_cycle--;
                    ...
                    // ADDED: try to blame a prior prefetch-caused eviction
                    // Both LOAD Misses and STORE (RFO) is counted here
                    //if ((cache_type == IS_L1D || cache_type == IS_L1I || cache_type == IS_L2C || cache_type == IS_LLC) &&

                    if ((cache_type == IS_L1D ) && (RQ.entry[index].type == LOAD || RQ.entry[index].type == RFO)) {
                      
                        uint64_t miss_tag = RQ.entry[index].full_virtual_address;
                        uint64_t block_address = miss_tag>>LOG2_BLOCK_SIZE;
                        if (ghost_hit_and_consume(0, block_address)) {
                            pollution_misses++;
                        }
                    }
                ...
                if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
                {
                    ...
                    int way = check_hit(&PQ.entry[index]);
                        ...
                        { 
                            ...
                            uint64_t pf_addr = PQ.entry[index].full_addr;
                            bool is_within_bounds = false;
                            uint32_t start =0;
                            uint32_t end=0;
                            for (const auto& it : malloc_map) {
                                start = it.first;
                                end = it.second;
                            if (pf_addr >= start && pf_addr < end) {
                               is_within_bounds = true;
                               break;
                            }
                           }
                           if (is_within_bounds) {
                            //std::cout << "[PREFETCH IN BOUNDS] PF_ADDR: 0x" << std::hex << pf_addr << "  start: " << std::hex << start << "  end: " << std::hex << end << std::dec << std::endl;       
                            }
                            else
                            {
                            //std::cout << "[PREFETCH OUTSIDE BOUNDS] PF_ADDR: 0x" << std::hex << pf_addr << "  start: " << std::hex << start << "  end: " << std::hex << end << std::dec << std::endl;
                            outside_bound = true;
                            }
                            prefetch_request++;
                       //std::cout << "Requests: " << prefetch_request
                       // << ", Hit-In: " << prefetch_hit_inside_bound
                       // << ", Hit-Out: " << prefetch_hit_outside_bound
                       // << ", Miss-In: " << prefetch_miss_inside_bound
                       // << ", Miss-Out: " << prefetch_miss_outside_bound
                       // << std::endl;
                    ...
                    if (way >= 0) { // prefetch hit
                        ...
                        if(outside_bound)
                        {
                            prefetch_hit_outside_bound++;
                        }
                        else
                        {
                            prefetch_hit_inside_bound++;
                        }
                        }
                        */
                        ...
                        if(cache_type == IS_L1D )
                        {
                            if(outside_bound)
                            {
                            prefetch_miss_outside_bound++;
                            }
                            else
                            {
                            prefetch_miss_inside_bound++;
                            }
                                    ...
                                    if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address))
                                        ...
                                        if(PQ.entry[index].pf_origin_level < fill_level)
                                        {
                                            if (cache_type == IS_L1D){
                                                ...
                                                // when calling l1d_prefetcher_operate on miss
                                               std::cout << "[L1D TRAIN] VA.full_addr=0x" << std::hex << PQ.entry[index].full_addr
                                               << "  VA.block=0x" << (PQ.entry[index].full_addr >> LOG2_BLOCK_SIZE)
                                               << "  PA.full=0x"   << PQ.entry[index].full_physical_address
                                               << "  cpu=" << std::dec << (int)prefetch_cpu
                                               << "  ip=0x" << std::hex << PQ.entry[index].ip << std::dec << "\n";
        ...
        int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata) /*, uint64_t prefetch_id)*/		//Neelu: commented. 
        {
            ...
            DP ( if (warmup_complete[cpu]) {cout << "entered prefetch_line, occupancy = " << PQ.occupancy << "SIZE=" << PQ.SIZE << endl; });
              ...
              // Kill if the prefetch would leave the base object's bounds
            // The entire object-bound guard (BTC lookup + out-of-bounds kill) is
            // compiled in only for the SAFE/guarded binary. Build with
            //   EXTRA_CFLAGS=-DHEAP_GUARD ./build_champsim.sh ...
            // for the guarded variant; omit it for the unguarded baseline. This
            // guarantees guard vs. no-guard binaries come from identical source.
            // btc_delay is added to the prefetch packet's event cycle below.
            // The bound check runs in parallel with the L1 TLB lookup (Fig. 4),
            // i.e. off the critical path, so it adds NO latency to the prefetch
            // path in either variant. We still exercise the BTC to report its
            // hit/miss rate, but keep btc_delay = 0.
            uint64_t btc_delay = 0;
#ifdef HEAP_GUARD
            if(cache_type == IS_L1D && btc != nullptr )
            {
                // Exercise the BTC for hit/miss statistics (off critical path).
                uint64_t tag = base_addr >> 6; // make it cache aligned
                uint64_t obj_base, obj_bound;
                if (!btc->lookup(tag, obj_base, obj_bound)) {
                    // [BTC MISS] Install for next time (bounds modeled by the
                    // oracle live_heap; BTC here models timing/occupancy only).
                    btc->insert(tag, /*base=*/0, /*bound=*/0);
                }

                // Kill if the prefetch would leave the base object's bounds
                if (!same_heap_object(base_addr, pf_addr)) {
                    pf_killed_by_heap_guard++;
                //    // Since it's killed, we just drop it entirely.
                    return 0;
                }
            }
#endif // HEAP_GUARD
            ...
            if (PQ.occupancy < PQ.SIZE) {
                ...
                //{
                //    std::cout << "[PF_PACKET] "
                //   << "pf_addr=0x" << std::hex << pf_addr
                //   << "  address(block)=0x" << pf_packet.address
                //   << "  full_addr(bytes)=0x" << pf_packet.full_addr
                //   << "  full_virtual(bytes)=0x" << pf_packet.full_virtual_address
                //   << std::dec << "  | address(block,dec)=" << pf_packet.address
                //   << '\n';
                ...
                pf_packet.event_cycle = current_core_cycle[cpu]+ btc_delay;
